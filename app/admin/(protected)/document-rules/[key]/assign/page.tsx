...
import { DocumentRulesWizard } from "@/components/admin/document-rules-wizard";
import {
  Card,
  CardDescription,
  CardHeader,
...
export default async function AdminDocumentRuleAssignPage({
  params,
}: {
  ...
  return (
    <AdminShell
      title={`Bulk assign · ${view.document.label}`}
      active="document-rules"
      subtitle="First-time setup or a bulk update. Everyday edits happen on a country."
    >
      <DocumentRulesWizard document={view.document} canWrite={view.canWrite} />
    </AdminShell>
  );
}
