...
- **Retention + cleanup (spec §11):** [`lib/applications/retain-required-documents.ts`](../lib/applications/retain-required-documents.ts) — invoked from the **idempotent payment webhook** in the same DB transaction as the first transition to `paymentStatus = paid` (see [`lib/payments/apply-payment-webhook-event.ts`](../lib/payments/apply-payment-webhook-event.ts)). **Partial retain:** for each of `passport_copy` and `personal_photo`, if the latest row is `uploaded_temp` **with** blob bytes, flip it to `retained`, set `retainedAt`, clear `tempExpiresAt`; **missing types are skipped** (paid with no pre-paid uploads is success with zero retained ids). If a type has `uploaded_temp` **without** bytes, good types are retained first, then the helper returns **`BLOB_BYTES_MISSING`** — the webhook **does not roll back** `paid`; it sets `adminAttentionRequired` and writes `payment_paid_docs_retain_failed_flagged`. Older audit rows may still reference legacy `MISSING_REQUIRED_DOCUMENT` from prior behavior. **`POST /api/internal/cleanup-drafts`** extended to delete orphaned temp blobs (`tempExpiresAt < now`, unpaid) and mark their documents `deleted`.
...
- **Apply UI:** [`components/apply/application-draft-panel.tsx`](../components/apply/application-draft-panel.tsx) — multipart upload slots for passport + personal photo, inline preview links, "Extract passport details" CTA, applicant review grid, readiness banner. Checkout / journey step gating uses **`paymentReadiness`** from `computeValidation` (profile + validation; uploads optional). Gating uses `GET /documents` (no client-only memory).
