...
export function ApplyJourneyStepBar({ step, totalSteps, title, subtitle, className, actions }: ApplyJourneyStepBarProps) {
  ...
  return <div className={cn("mx-auto mb-6 flex w-[calc(100%-2rem)] max-w-3xl items-center gap-3 rounded-2xl border border-secondary/20 bg-card px-4 py-3 shadow-[0_10px_30px_rgba(1,32,49,0.07)]", className)}>
    <span className="bg-secondary text-white shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide sm:text-xs">Step {step}/{totalSteps}</span>
    ...
    <button type="button" onClick={() => setHidden(true)} className="border-border text-muted-foreground hover:bg-muted hover:text-foreground flex size-8 shrink-0 items-center justify-center rounded-full border" aria-label="Hide progress summary"><X className="size-4" aria-hidden /></button>
  </div>;
}
